...
dataset_names = [
    "Refresh page to view example data",
    "Refresh page to view example data",
]
...
# UPDATE PLOTS WHEN NODES ARE HOVERED OVER #
@app_dash.callback(
...
# UPDATE PLOTS WHEN NODES ARE SELECTED #
@app_dash.callback(
...
def update_aa_plot(
    ...
    if (
        first_dataset == "Refresh page to view example data"
        or second_dataset == "Refresh page to view example data"
    ):
        raise PreventUpdate
    ...
    current_data = DataStore.objects().get(
        session_token=str(session["data_generated"].strip())
    )
    ...
    current_layout = curr_aa_plot.generate_plot(
        metric1,
        metric2,
        first_dataset,
        second_dataset,
