...
class TreeController:
    ...
    def _get_similar_node_for_node(
        self, tree_known_ancs, tree_unknown_ancs, node_label, same_tree
    ):
        intersection = (
            []
        )  # The intersection between the known and unknown trees, array of tree node objects
    ...
    def get_similar_nodes_between_trees(
        self, tree_known_ancs, tree_unknown_ancs, same_tree
    ):
        intersection = self._get_intersection(
            tree_known_ancs, tree_unknown_ancs
        )  # The intersection between the known and unknown trees, array of tree node objects
        ...
        for label, ancestor_node in nodes.items():
            ...
            if len(ancestor_intersection) < 1:
                ...
                self._score_nodes(
                    ancestor_intersection, tree_unknown_ancs_root, ancestor_node
                )
    ...
    def _score_nodes(self, extent_id_list, node, ancs_node_compared_with):
        ...
        for child_tree_node in node.get_children():
            score += self._score_nodes(
                extent_id_list, child_tree_node, ancs_node_compared_with
            )
            if self.breakout:
                return 0.0
        ...
        if not self.best_node:
            ...
            if node.get_other_extent_count() == self.best_node.get_other_extent_count():
                ...
                this_dist_diff = abs(
                    node.get_dist_to_root() - ancs_node_compared_with.get_dist_to_root()
                )
                best_node_dist_diff = abs(
                    self.best_node.get_dist_to_root()
                    - ancs_node_compared_with.get_dist_to_root()
                )
