...
def print_record_overview(records):
    ...
    print(
        "There are %s sequences and the average length of sequence is %d"
        % (len(records), total_len / len(records))
    )
...
def build_species_count(*exclude_on, records, length=0):
    ...
    for record in records.values():
        if len(str(record.seq)) > length and not any(
            exclude in record.description for exclude in exclude_on
        ):
...
def subset_records(*header_terms, records, length=0, mode="exclude", ignore_case=True):
    ...
    for record in records.values():
        ...
        if mode == "exclude":
            ...
            elif len(str(record.seq)) > length and not any(
                term in record.description for term in header_terms
            ):
                new_records[record.name] = record
            ...
            if ignore_case and len(str(record.seq)) > length:
                ...
                if any(term in record_lower for term in header_lower):
                    new_records[record.name] = record
            elif len(str(record.seq)) > length and any(
                term in record.description for term in header_terms
            ):
                new_records[record.name] = record
...
def add_species_from_header(records):
    ...
    for record in records:
        if "[" in records[record].description:
            species_name = (
                records[record].description.split(">")[0].split("[")[1].split("]")[0]
            )
...
def report_records_without_species(records):
    ...
    if missing:
        print(
            "The following records do not have species annotated", [x for x in missing]
        )
    else:
            ...
            for seq in cluster:
                ...
                if "*" in seq:
                    seq_info = (
                        ...
                        + seq_id
                        + " "
                        + records[seq_id].annotations["Species"]
                    )
                    seq_list.insert(
                        0, seq_info
                    )  # Add the retained sequence to the start of the seq list
                else:
                    seq_info = (
                        "Removed "
                        + seq_id
...
def get_species_names(records, min_length=0, counts=False):
    ...
    for k, v in records.items():
        if len(v) >= min_length:
            species_names.append(k) if not counts else species_names.append(
                k + " " + str(len(v))
            )
...
def plot_record_number(records, plot_type, min_length=0):
    ...
    if plot_type == "Bar":
        ...
        data = [
            Bar(x=list(plot_records.keys()), y=[len(x) for x in plot_records.values()])
        ]
...
def subset_on_motif(records, motif, retain_motif_seqs=True):
    ...
    for record in records.values():
        if (len(pattern.findall(str(record.seq))) > 0 and retain_motif_seqs) or (
            len(pattern.findall(str(record.seq))) == 0 and not retain_motif_seqs
        ):
