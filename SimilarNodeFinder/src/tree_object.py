...
class TreeObject:
    ...
    def _parse_newick(self, newick_str, parent, node_ids, count, next_id):
        ...
        if start_idx == -1 and end_idx == -1:
            ...
            node = self._parse_internal_newick(
                embed, tail, parent, node_ids, count, next_id
            )
            next_id += 1
    ...
    def _get_dist(self, sub_str, idx_start):
        ...
        try:
            ...
            print(
                "There was a non-digit number that was attempted to be parsed: ",
                sub_str,
            )
            return 0.0
    ...
    def _parse_internal_newick(self, embed, tail, parent, node_ids, count, next_id):
        ...
        while comma != -1:
            ...
            node.add_child(
                self._parse_newick(to_process, node, node_ids, count, next_id)
            )
            next_id += 1
